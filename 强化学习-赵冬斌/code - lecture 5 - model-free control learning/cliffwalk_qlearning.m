...
for t = 1:N
    ...
    while ~terminal
        ...
        [sn,r,terminal] = cliff_walk(s,a); 
        delta = r+gamma*max(Q(:,sn(1),sn(2)))-Q(a,s(1),s(2)); 
        Q(a,s(1),s(2)) = Q(a,s(1),s(2)) + alpha*delta; 
        s = sn; 
        draw_gridworld(s,Q,draw_flag)
        len = len+1; 
    end
...
function [sn,r,terminal] = cliff_walk(s,a)
% cliff walk model
% input:
...
% restrict state axis 
sn(1) = min(12,max(1,sn(1))); 
sn(2) = min(4,max(1,sn(2))); 

% terminal state and reward
if sn(1)>=2 && sn(1)<=11 && sn(2)==1
    terminal = 1; 
    r = -100; 
elseif sn(1)==12 && sn(2)==1
    terminal = 1; 
    r = -1; 
else
    terminal = 0; 
...
% cliff
rectangle('position',[1,0,10,1],'facecolor','k'); 

% start and goal point 
text(0.3,0.5,'S','fontsize',20); 
text(11+0.3,0.5,'G','fontsize',20); 
